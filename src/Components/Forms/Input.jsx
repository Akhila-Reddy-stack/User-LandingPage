    ...
    return (
        <Fragment>
            <div className="form-group">
                {props.label && <label htmlFor={field} style={{ fontWeight: '600', fontSize: '16px', color: '#0268ba' }}>{props.label}:</label>}
                <input
                    {...rest}
                    id={field}
                    ref={forwardedRef}
                    // value={!value && value !== 0 ? '' : value}
                    className={classnames(`form-control form-control-sm ${className}`, { "is-invalid": fieldState.error })}
                    onChange={e => {
